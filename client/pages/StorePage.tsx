    ...
    <section className="pt-32 sm:pt-36"><div className="mx-auto max-w-7xl px-5 sm:px-8">
      ...
      <div className="mt-10 space-y-8">{storeSets.map((set) => <article className="overflow-hidden border border-white/10 bg-[#141414]" key={set.name}>
        <div className="grid lg:grid-cols-[minmax(0,1.06fr)_minmax(0,0.94fr)]">
          <div className="relative min-h-[25rem] overflow-hidden bg-[#0b0b0b] p-5 sm:p-8" style={{ backgroundImage: `url(${STORE_BACKDROP_DATA_URL})`, backgroundPosition: "center", backgroundSize: "cover" }}>
            ...
            <div className="pointer-events-none absolute inset-x-4 bottom-16 top-16 grid grid-cols-2 items-center gap-2 opacity-95 sm:inset-x-10"><img alt={`${set.name} — camisa`} className="h-full w-full object-contain" src={set.images[0]} /><img alt={`${set.name} — bermuda`} className="h-full w-full object-contain" src={set.images[1]} /></div><div className="pointer-events-none absolute inset-0 bg-[linear-gradient(0deg,rgba(10,10,10,0.9),transparent_52%)]" />
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-1">{set.pieces.map((item) => <article className="group flex min-h-[19rem] flex-col border-l border-t border-white/10 bg-[#111111] first:border-t-0 sm:first:border-t lg:first:border-t-0" key={item.id}>
            <div className="relative flex min-h-[12rem] flex-1 items-center justify-center overflow-hidden bg-[#0b0b0b]" style={{ backgroundImage: `url(${STORE_BACKDROP_DATA_URL})`, backgroundPosition: "center", backgroundSize: "cover" }}><img alt={item.name} className="h-full w-full object-contain p-3 transition-transform duration-500 group-hover:scale-[1.035]" src={item.image} /></div>
