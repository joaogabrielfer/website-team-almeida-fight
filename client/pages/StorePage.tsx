      ...
      <div className="mt-10 space-y-8">{storeSets.map((set) => <article className="overflow-hidden border border-white/10 bg-[#141414]" key={set.name}>
        <div className="grid lg:grid-cols-[minmax(0,1.06fr)_minmax(0,0.94fr)]">
          <div className="overflow-hidden bg-[#0b0b0b]" style={{ backgroundImage: `url(${STORE_BACKDROP_DATA_URL})`, backgroundPosition: "center", backgroundSize: "cover" }}>
            ...
            <div className="border-t border-white/15 bg-[#0d0d0d] p-5 sm:p-8"><div className="flex items-center gap-1.5"><p className={set.collection === "Edição exclusiva" ? "inline-flex shrink-0 bg-[#d4af37] px-3 py-2 text-[0.62rem] font-black uppercase tracking-[0.16em] text-[#0d0d0d]" : "shrink-0 text-[0.62rem] font-black uppercase tracking-[0.16em] text-[#f3d66b]"}>{set.collection}</p>{set.collection === "Personalizado" && <span className="inline-flex shrink-0 whitespace-nowrap bg-[#d4af37] px-3 py-2 text-[0.62rem] font-black uppercase tracking-[0.1em] text-[#0d0d0d] shadow-[0_5px_18px_rgba(212,175,55,0.16)]">Coloque o seu nome</span>}</div><h2 className="mt-5 max-w-sm font-['Teko'] text-5xl font-bold uppercase leading-[0.82] text-white sm:text-6xl">{set.name}</h2><div className="mt-6 flex items-end justify-between gap-4 border-t border-white/20 pt-5"><div><p className="text-xs text-white/70">{set.description}</p><strong className="mt-2 block font-['Teko'] text-4xl font-bold leading-none text-[#e3c03b]">{priceLabel(set.price)}</strong></div><BuyButton dark product={set.name} /></div></div>
          </div>
