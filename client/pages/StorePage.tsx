...
function ProductCarousel({ alt, views }: { alt: string; views: readonly string[] }) {
  const [activeView, setActiveView] = useState(0);

  useEffect(() => {
    if (views.length < 2) return;
    const timer = window.setInterval(() => setActiveView((current) => (current + 1) % views.length), 2800);
    return () => window.clearInterval(timer);
  }, [views]);

  return <div className="relative h-full w-full">
    {views.map((view, index) => <img alt={`${alt} — ${index === 0 ? "frente" : "costas"}`} className={`absolute inset-0 h-full w-full object-contain p-1 transition-opacity duration-500 ${index === activeView ? "opacity-100" : "opacity-0"}`} key={view} src={view} />)}
    {views.length > 1 && <><span className="absolute left-3 top-3 border border-[#d4af37]/50 bg-[#0d0d0d]/90 px-2 py-1 text-[0.5rem] font-black uppercase tracking-[0.14em] text-[#f3e5ab]">{activeView === 0 ? "Frente" : "Costas"}</span><div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1.5">{views.map((view, index) => <button aria-label={`Mostrar ${index === 0 ? "frente" : "costas"} de ${alt}`} aria-pressed={index === activeView} className={`h-1.5 rounded-full transition-all ${index === activeView ? "w-5 bg-[#d4af37]" : "w-1.5 bg-white/45 hover:bg-white"}`} key={view} onClick={() => setActiveView(index)} type="button" />)}</div></>}
  </div>;
}
      ...
      <div className="mt-10 space-y-8">{storeSets.map((set) => <article className="overflow-hidden border border-white/10 bg-[#141414]" key={set.name}>
        <div className="grid lg:grid-cols-[minmax(0,1.06fr)_minmax(0,0.94fr)]">
          <div className="overflow-hidden bg-[#0b0b0b]" style={{ backgroundImage: `url(${STORE_BACKDROP_DATA_URL})`, backgroundPosition: "center", backgroundSize: "cover" }}>
            ...
            <div className="border-t border-white/10 bg-[#111111] p-5 sm:p-8"><p className="text-[0.6rem] font-black uppercase tracking-[0.2em] text-[#d4af37]">{set.collection}</p>{set.collection === "Personalizado" && <span className="mt-4 block w-fit border border-[#d4af37]/60 bg-[#d4af37]/10 px-3 py-1.5 text-[0.58rem] font-black uppercase tracking-[0.14em] text-[#f3e5ab]">Coloque o seu nome</span>}<h2 className="mt-5 max-w-sm font-['Teko'] text-5xl font-bold uppercase leading-[0.82] text-white sm:text-6xl">{set.name}</h2><div className="mt-6 flex items-end justify-between gap-4 border-t border-white/15 pt-5"><div><p className="text-xs text-white/55">{set.description}</p><strong className="mt-2 block font-['Teko'] text-4xl font-bold leading-none text-[#d4af37]">{priceLabel(set.price)}</strong></div><BuyButton dark product={set.name} /></div></div>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-2">{set.pieces.map((item) => <article className="group flex flex-col border-l border-t border-white/10 bg-[#111111] first:border-t-0 sm:first:border-t lg:first:border-t" key={item.id}>
            <div className="relative flex h-64 items-center justify-center overflow-hidden bg-[#0b0b0b] sm:h-72 lg:h-72" style={{ backgroundImage: `url(${STORE_BACKDROP_DATA_URL})`, backgroundPosition: "center", backgroundSize: "cover" }}><ProductCarousel alt={item.name} views={"views" in item ? item.views : [item.image]} /></div>
            <div className="p-5">{item.collection.startsWith("Personalizado") && <span className="mb-4 inline-flex border border-[#d4af37]/55 bg-[#d4af37]/10 px-2.5 py-1.5 text-[0.55rem] font-black uppercase tracking-[0.13em] text-[#f3e5ab]">Coloque o seu nome</span>}<h3 className="font-['Teko'] text-3xl font-bold uppercase leading-none text-white">{item.name}</h3><p className="mt-2 text-xs leading-5 text-white/55">{item.description}</p><div className="mt-4 flex items-center justify-between gap-3 border-t border-white/10 pt-4"><strong className="font-['Teko'] text-3xl font-bold leading-none text-[#d4af37]">{priceLabel(item.price)}</strong><BuyButton product={item.name} /></div></div>
          </article>)}</div>
        </div>
      </article>)}</div>
    </div></section>
