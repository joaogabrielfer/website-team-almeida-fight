...
function ProductCarousel({ alt, views }: { alt: string; views: readonly string[] }) {
  ...
  return <div className="relative h-full w-full">
    {views.map((view, index) => <img alt={`${alt} — ${index === 0 ? "frente" : "costas"}`} className={`absolute inset-0 h-full w-full object-contain p-3 transition-opacity duration-700 ${index === activeView ? "opacity-100" : "opacity-0"}`} key={view} src={view} />)}
      ...
      <div className="mt-10 space-y-8">{storeSets.map((set) => <article className="overflow-hidden border border-white/10 bg-[#141414]" key={set.name}>
        <div className="grid lg:grid-cols-[minmax(0,1.06fr)_minmax(0,0.94fr)]">
          <div className="overflow-hidden bg-[#0b0b0b]" style={{ backgroundImage: `url(${STORE_BACKDROP_DATA_URL})`, backgroundPosition: "center", backgroundSize: "cover" }}>
            ...
            <div className="border-t border-white/10 bg-[#111111] p-5 sm:p-8"><p className="text-[0.6rem] font-black uppercase tracking-[0.2em] text-[#d4af37]">{set.collection}</p>{set.collection === "Personalizado" && <span className="mt-3 inline-flex border border-[#d4af37]/60 bg-[#d4af37]/10 px-2.5 py-1 text-[0.52rem] font-black uppercase tracking-[0.14em] text-[#f3e5ab]">Coloque o seu nome</span>}<h2 className="mt-3 max-w-sm font-['Teko'] text-5xl font-bold uppercase leading-[0.82] text-white sm:text-6xl">{set.name}</h2><div className="mt-6 flex items-end justify-between gap-4 border-t border-white/15 pt-5"><div><p className="text-xs text-white/55">{set.description}</p><strong className="mt-2 block font-['Teko'] text-4xl font-bold leading-none text-[#d4af37]">{priceLabel(set.price)}</strong></div><BuyButton dark product={set.name} /></div></div>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-2">{set.pieces.map((item) => <article className="group flex flex-col border-l border-t border-white/10 bg-[#111111] first:border-t-0 sm:first:border-t lg:first:border-t" key={item.id}>
            <div className="relative flex h-48 items-center justify-center overflow-hidden bg-[#0b0b0b] lg:h-52" style={{ backgroundImage: `url(${STORE_BACKDROP_DATA_URL})`, backgroundPosition: "center", backgroundSize: "cover" }}><ProductCarousel alt={item.name} views={"views" in item ? item.views : [item.image]} /></div>
            <div className="p-5">{"views" in item && <span className="mb-3 inline-flex border border-[#d4af37]/45 px-2 py-1 text-[0.48rem] font-black uppercase tracking-[0.13em] text-[#f3e5ab]">Coloque o seu nome</span>}<h3 className="font-['Teko'] text-3xl font-bold uppercase leading-none text-white">{item.name}</h3><p className="mt-2 text-xs leading-5 text-white/55">{item.description}</p><div className="mt-4 flex items-center justify-between gap-3 border-t border-white/10 pt-4"><strong className="font-['Teko'] text-3xl font-bold leading-none text-[#d4af37]">{priceLabel(item.price)}</strong><BuyButton product={item.name} /></div></div>
          </article>)}</div>
        </div>
      </article>)}</div>
    </div></section>
